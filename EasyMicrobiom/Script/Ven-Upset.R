...
# VenUpset(otu = "./otutab.txt",tax = "./taxonomy.txt",map = "./metadata.tsv",rep = 6)

VenUpset = function(otu = "./otutab.txt",tax = "./taxonomy.txt",map = "./metadata.tsv",rep = 6){
  path = "./VenUpset/"
  dir.create(path)
  library("phyloseq")
  library(UpSetR)
  ...
  if (ven_pick[[1,6]] != 0) {
    ...
    filename44 = paste(path,"./UpSet",".jpg",sep = "",collapse="_")
    jpeg(file=filename44)
    upset(ven2, sets = colnames(ven2),
          number.angles = 30, point.size = 2, line.size = 1,
          mainbar.y.label = "OTU", sets.x.label = "OTU Per Treatment",
          text.scale = c(2, 2, 2,2, 2, 2),mb.ratio = c(0.7, 0.3),order.by = "freq",keep.order = TRUE,
